...
class DijkstrasAlgorithm { 
    ...
    { 
        ...
        { 
            shortestDistances[vertexIndex] = Integer.MAX_VALUE;    // Initialize all distances as INFINITE
            visited[vertexIndex] = false; 						// initially no vertex has been finalized
        } 
        ...
        for (int i = 1; i < nVertices; i++) // external for loop
        { 
  
            // Pick the minimum distance vertex from the set of vertices not yet processed. 
        	// nearestVertex is always equal to startNode in first iteration. 
            int nearestVertex = -1; 
            int overallShortDist = Integer.MAX_VALUE; 
            for (int vertexIndex = 0; vertexIndex < nVertices;  vertexIndex++) 
            { 
                if (!visited[vertexIndex] && shortestDistances[vertexIndex] < overallShortDist)  
                { 
                    nearestVertex = vertexIndex; 
                    overallShortDist = shortestDistances[vertexIndex]; 
                } 
            } 
            ...
            { 
                ...
                if (edgeDistance > 0
                    && ((overallShortDist + edgeDistance) < shortestDistances[vertexIndex]))  
                { 
                    parents[vertexIndex] = nearestVertex; 
                    shortestDistances[vertexIndex] = overallShortDist + edgeDistance; 
                } 
            } 
        } 
